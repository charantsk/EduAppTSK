...
def calculate_score(form_id, responses): 
    ...
    for question in questions:
        ...
        if str(question.id) in responses_dict:
            student_answer = str(responses_dict[str(question.id)]).strip()
            ...
            # For other question types
                ...
                if correct_answer == 'null' or student_answer.lower() == correct_answer.lower():
                    total_score += question.points
...
def init_routes(app):
    ...
    @login_required
    @requires_role(UserRole.STAFF)  # Optional: Restrict to staff/admin users
    def get_all_users():
