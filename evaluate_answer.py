...
def check_answer(question, correct_answer, keywords, student_answer, threshold=0.5):
    """
    Check if a student's answer is correct using minimal NLP (keywords + Jaccard similarity).
    ...
    # Extract words (lemmas) from the student answer
    student_tokens = {token.lemma_.lower() for token in nlp(student_answer) if token.is_alpha}
    ...
    union = student_tokens.union(correct_tokens)
    similarity = len(intersection) / len(union) if union else 0

    # Check keywords
    keywords_found = sum(1 for keyword in keywords if keyword.lower() in student_tokens)
